...
var left=$(".left")[0];
var right=$(".right")[0];
var arrColor=["#65d9fe","#fe3794","#0172ff","#e8e8e8","#e8e8e8","#e8e8e8"]
var num=0;
function move(type){
	if(type=="r"){
			num++;
	...
	animate(bbackground,{opacity:1},400,Tween.Linear);
...
for(var i=0;i<btns.length;i++){
	...
	btns[i].onmouseover=function(){
		for(var j=0;j<as.length;j++){
			...
			as[j].style.filter="alpha(opacity=0)";
			bbackground.style.opacity=10;
			bbackground.style.filter="alpha(opacity=10)";
			btns[j].style.background="#000";
		}
		as[this.index].style.opacity="1";
		bbackground.style.opacity="1";
